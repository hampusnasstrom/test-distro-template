![docker image](https://github.com/hampusnasstrom/test-distro-template/actions/workflows/docker-publish.yml/badge.svg)
...
2. Clone the repository or download the repository as a zip file.
    ...
    ```sh
    git clone https://github.com/hampusnasstrom/test-distro-template.git
    cd test-distro-template
    ```

    or

    ```sh
    curl-L -o test-distro-template.zip "https://github.com/hampusnasstrom/test-distro-template/archive/main.zip"
    unzip test-distro-template.zip
    cd test-distro-template
    ```
...
your `docker-compose.yaml` with `ghcr.io/hampusnasstrom/test-distro-template:main` for the services
...
```
docker pull ghcr.io/hampusnasstrom/test-distro-template/jupyter:main
```
...
_I get an_ `Error response from daemon: Head "https://ghcr.io/v2/hampusnasstrom/test-distro-template/manifests/main": unauthorized`
